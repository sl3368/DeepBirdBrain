...
for i_batch = start:end_batch
	filepath='/vega/stats/users/sl3368/LifeClef_Segmented_Songs/';
	filenames=dir('/vega/stats/users/sl3368/LifeClef_Segmented_Songs/');
	...
	save(sprintf('/vega/stats/users/sl3368/Data_LC/nopad_processing/batchSizes%d.mat',i_batch),'batchSizes')
	save(sprintf('/vega/stats/users/sl3368/Data_LC/nopad_processing/songSums%d.mat',i_batch),'songSums')
	save(sprintf('/vega/stats/users/sl3368/Data_LC/nopad_processing/outSpectrum%d.mat',i_batch),'outSpectrum','-v7.3')
...
batchSize = 4000;
for i_batch = start:end_batch
       load(sprintf('/vega/stats/users/sl3368/Data_LC/nopad_processing/batchSizes%d.mat',i_batch))
       if i_batch < 1410 
           batchInds = (i_batch-1)*batchSize+1:i_batch*batchSize;
...
batchSize = 4000;
for i_batch = start:end_batch
     load(sprintf('/vega/stats/users/sl3368/Data_LC/nopad_processing/songSums%d.mat',i_batch))
     if i_batch < 1410 
         batchInds = (i_batch-1)*batchSize+1:i_batch*batchSize;
...
save /vega/stats/users/sl3368/Data_LC/nopad_processing/allsongMean allsongMean

% Standard deviation
sqrt((1/(size(songSums,1)-1))*sum((bsxfun(@minus,songSums,mean(songSums))).^2))
stdStep=[];
load /vega/stats/users/sl3368/Data_LC/nopad_processing/allsongMean.mat
for i_batch = start:end_batch
	load(sprintf('/vega/stats/users/sl3368/Data_LC/nopad_processing/outSpectrum%d.mat',i_batch))
	if i_batch<141
...
save /vega/stats/users/sl3368/Data_LC/nopad_processing/stdStep stdStep
allsongSTD = sqrt((1/(sum(batchSize_all)-1))*sum(stdStep));
save /vega/stats/users/sl3368/Data_LC/nopad_processing/allsongSTD allsongSTD

%% Pad songs

start=1;
end_batch=25;
padded_song_length=1200;
for i_batch=start:end_batch

	batchSize = 4000; %number of songs in each saved portion of the spectogram
	...
	%% Normalize
	load /vega/stats/users/sl3368/Data_LC/nopad_processing/allsongSTD
	load /vega/stats/users/sl3368/Data_LC/nopad_processing/allsongMean
	...
	save(sprintf('/vega/stats/users/sl3368/Data_LC/NopadNormData/LC_stim_%d.mat',i_batch),'stimulus_zscore','useMask','-v7.3')
	save(sprintf('/vega/stats/users/sl3368/Data_LC/NopadNormData/LC_inds_%d.mat',i_batch),'trainInds_small','newInds','-v7.3')
	disp('Save Complete..')
end
