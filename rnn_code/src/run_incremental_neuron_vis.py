...
for i in range(len(region)):
    arguments = '-vREGION='+region[i]+',HELDOUT='+str(heldout[i])+',NEURON='+str(neuron[i])+',ITERATIONS='+str(iterations)+',HIDDEN='+str(hidden)+',STEPSIZE='+str(step)+',SONGSIZE='+str(songsize)
    subprocess.call(['qsub',arguments,'incremental_neuron_visualizations.sh'])
