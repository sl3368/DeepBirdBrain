...
for i in xrange(half,song_size-1,step):
    y_gen_5[i] = ygen_model(y_gen_truth[0:i-1])[i-2]
    for s in xrange(1,step):
	y_gen_5[i+s] = ygen_model(y_gen_5[0:i+s-1])[i+s-2]

step = 10
y_gen_10 = data_set_x[index * song_size:((index + 1) * song_size - 1)].eval()
for i in xrange(half,song_size-1,step):
    y_gen_10[i] = ygen_model(y_gen_truth[0:i-1])[i-2]
    for s in xrange(1,step):
	y_gen_10[i+s] = ygen_model(y_gen_10[0:i+s-1])[i+s-2]


step = 25
y_gen_25 = data_set_x[index * song_size:((index + 1) * song_size - 1)].eval()
for i in xrange(half,song_size-1,step):
    y_gen_25[i] = ygen_model(y_gen_truth[0:i-1])[i-2]
    for s in xrange(1,step):
	y_gen_25[i+s] = ygen_model(y_gen_25[0:i+s-1])[i+s-2]
...
savefilename = savefilename_pre + str(index)+'_batch_5.analysis'
