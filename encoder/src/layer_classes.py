...
class LeNetConvPoolLayer(object):
	...
	if pool:
	    self.F_prime = self.output.shape
            self.F = self.output.reshape((n_filters,flattened_length))
        else:
	    self.F_prime = self.convolution.shape
            self.F = self.convolution.reshape((n_filters,flattened_length))

	#find g_matrix by reshaping then multiplying with transpose
        self.G = T.dot(self.F,self.F.T)
